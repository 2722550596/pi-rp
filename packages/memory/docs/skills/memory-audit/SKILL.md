...
你不需要扫描整棵记忆树。起手从 awaken 清单常驻的、对话中读取过的、diagnostic 报出来的记忆开始。视野会在整理过程中自然扩展——检查一条记忆时，你可能读到它引用的其他节点、注意到"相关联想"里的名字、发现疑似重复的 URI。读出来了就进了视野，发现了问题就跟下去。

**建议的起手：**

1. **awaken 清单（常驻记忆优先）**。`recall("MEM://wakeup")` 或 `awaken(action:"list")`。这些记忆每一轮对话都占你的上下文带宽，是质量杠杆最高的地方——一条平庸的记忆常驻，等于每次呼吸都带着一块石头。

2. **对话中触及过的记忆节点**。如果刚结束一轮对话，从那次对话中 recall/retrieve 过的记忆开始，趁上下文还热。需要核对记忆与事实是否相符时，用 `retrace` 翻原文日志（`retrace(uri=...)` 看该记忆锚定的原文区间，`retrace(query=...)` 全文搜索）——记忆是解读，原文是铁证。

3. **`MEM://diagnostic/<domain>`**（首选的系统诊断）。调用 `recall("MEM://diagnostic/core")` 。它会直接报出三类已确诊的病灶：
   - **Stale memories**：超过约 30 天未被你主动想起的节点。注意这里的"想起"只统计你主动调用 `recall`/`retrieve`——**自动注入、slot 渲染、MEM 视图浏览都不算**。所以"它没有被读"有两种截然不同的病因，诊断前必须先区分：是"从未被主动想起但其实常随对话自动注入"（它可能活得很好），还是"连自动注入都轮不到它"（它真的死了）。对每一条，先问自己：**为什么它没有被读？** 在你回答这个问题之前，去读它的内容、disclosure、parent 位置。用你自己的判断去推断原因，然后再根据你的诊断选择子技能。以下是一些你可能发现的模式，但它们只是思考的拐杖，不是穷举：
     - 它是死数据——读不读你的行为都不会变（→ `dead-data-purge`）
     - 它的 disclosure 失效或 parent 放错了，导致实战中永远不会被想起（→ `discoverability`）
     - 它的 importance 虚高，它没重要到需要那个分数（→ `discoverability`）
   ...
   - **Placeholder（占位节点）**：memorize 时自动补出来的空父链。它们是"等待被填实"的骨架，不是垃圾——该填的填（带正文写入即转正），该拆的拆，无人认领的清理掉。

4. **`MEM://forgotten/<domain>`（沉睡打捞）**：调用 `recall("MEM://forgotten/history/10")` 等。它按"沉睡最久"排出活记忆——沉睡基准是你的主动想起时间。这是做梦式审计的替代：不是随机抽签，而是确定性地把被遗忘最久的记忆捞到你面前。**警惕你的审计动作本身会污染这份名单**：recall 一个父节点会把整棵子树的访问时间一起刷新，读一条就洗白一条。所以先扫列表、记下最沉睡的那几条 URI，再逐个深入，不要边扫边读。

5. **`MEM://index/<domain>`**。扫一眼结构，挑出"看着就觉得不对劲"的节点。

6. **`TEMP://` 动态区**（收到整理通知时）。动态区是草稿堆场，堆到阈值引擎会提醒你。整理目标是把 TEMP 清到零：有价值的归位（`memorize`/`consolidate`），过期的删除。
...
| disclosure写成了自我觉察式（"当我感到..."）；parent放错导致实战想不起来；子节点>10；缺别名入口导致翻了好几个节点才找到 | `memory-audit-discoverability` |
...
5. **行动蓝图**：为了确保未来的我能像本能一样执行这个理想选择，我需要怎么改这条记忆？用哪把刀——`revise` 整条重写、`forget` 清掉、`relocate` 挪位置，还是连 disclosure 一起改？
