 ...
 */
export function createMemorySlots(store: MemoryStore): MemorySlotDefinition[] {
	const awaken: MemorySlotDefinition = {
		...
		render: () => {
			...
			for (const uri of uris) {
				...
				if (!node || node.is_stub) continue; // 对账：已删/失效 uri 自动剔除
	...
	const recent: MemorySlotDefinition = {
		...
		render: (context) => {
			...
			const nodes = store.listRecentNodes(Math.max(options.snippetCount ?? 10, 1));
			...
			if (rawCount > 0) {
				...
				const upto = last.m ?? 0;
				const rows = upto > 0 ? store.listRaw(Math.max(1, upto - rawCount + 1), upto) : [];
	...
	const index: MemorySlotDefinition = {
		...
		render: () => {
			...
			for (const domain of store.listDomains()) {
				const roots = store.listNodes({ domain }).filter((x) => !x.is_stub && x.parent_id === null);
