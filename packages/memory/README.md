...
/memories web           # 打开本地记忆浏览器(未启动则自动拉起)
```

### 2.4 记忆浏览器(本地 Web 界面)

给使用者用的图形界面:在浏览器里浏览记忆树、翻原文日志、看系统视图、**直接编辑记忆**。
它不是 TUI 的替代品,而是"另一块屏幕上一直开着"的那扇窗。

**打开方式**——两种等价:

```bash
# A. TUI 内(推荐):借用当前会话的库路径,自动拉起并打开浏览器
/memories web

# B. 独立命令(不依赖任何 pi 会话)
pi-memory-web --db ./world/magnolia/elias/.pi/memory.db
pi-memory-web --db ./memory.db --port 9000 --open   # 指定端口 / 直接开浏览器
```

#### 它是一个独立进程,不跟着会话走

这一点是刻意的:**`/memories web` 是启动器,不是服务**。它向 web 进程借出的只有
"用哪个库"这一条信息,不借生命周期。
...
/memories web
   ├─ 探测:起始端口上已有服务本库的实例?
   │     ├─ 有  → 直接开浏览器(不重复拉起)
   │     └─ 无  → spawn 一个 detached 独立进程
   └─ 等它就绪 → 开浏览器 → 命令结束
```

- web 进程以 `detached` 启动并立即与 pi 脱钩;**会话退出、你关掉 TUI,浏览器照常开着**,直到你手动结束它(`pkill -f pi-memory-web`,或关掉它的终端)。
- 因此也**不需要**为了看记忆而保留会话 —— 退出会话前把浏览器开好即可。

**端口与复用**:默认从 `8788` 起(`PI_MEMORY_WEB_PORT` 可改),向后找 10 个位置。
复用判据不是"端口上有人监听",而是**"监听的那个实例正服务着我要看的库"**
(比对 `/api/meta` 的 `db_path`)——同一个 world 下作家与角色各有一个库,
只凭端口占用就复用会把你静默带到**别人的库**上。库不匹配时让路,换端口另起。

#### 能做什么

左侧栏是固定的 9 个视图(不提供 `MEM://` 手输框),顶上有搜索框:

| 视图 | 内容 |
|---|---|
| 记忆树 | domain 分层浏览,点节点看全文与子节点 |
| 节点 | 单条记忆全文、元数据、修订史(可回滚)、关联边 |
| 搜索 | 关键词检索(结果卡片可点进节点) |
| 原文 | 原文日志(`active` 状态一眼可辨,离枝行有斜纹标记) |
| 动态区 | TEMP 暂存内容 |
| 系统视图 | forgotten / recent / wakeup / glossary / index / diagnostic / timeline |
| 审计 | 审计流只读列表 |
| 编辑 | 改正文 / 重要度 / 想起条件 / 世界时间,增删触发词,加关联,移动,删除 |
| 已删除 | 已删记忆清单与恢复 |

写入口在**节点页**的「编辑」「删除」按钮,以及**侧栏顶部**的「＋ 新建记忆」。
经 Web 界面做的编辑,署名记为 `manual`(与角色自己写的一样是"人写的"),不是模型产物。

#### 注意

- 默认只绑 `127.0.0.1`。改 `--host` 绑到非回环地址会打印醒目警告 —— 同网段任何主机都能读写你的记忆库。
