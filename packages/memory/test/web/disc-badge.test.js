 ...
 *    `src/web/assets/views/ui.js` 是浏览器侧的纯 ESM，无构建步骤。仓库的「相对 import 必须
 ...
 *   `el`/`append`（`ui.js`，均显式跳过 `null` 子项）把这个 null 安静地丢掉。若将来有人重写其中之一
 *   而不再跳过 `null`，界面上会出现字面量 **"null"** 文本 —— 本文件把那个守卫钉进测试。
 *
 * 非空性：`discBadge`/`discChip` 自 2026-09 重设计起**逐字迁入 `ui.js`**（views.js 不再持有
 *   第二份）；import 失败即红。第二组断言进一步要求**有值**时**必须**产出 `mw-disc--entry` 节点
 *   （否则"返回 null"那条会在一个恒返回 null 的实现上假绿）。
 */
import { afterAll, beforeEach, describe, expect, it } from "vitest";
import { discBadge, discChip, foldWs } from "../../src/web/assets/ui.js";

/**
 * 最小 DOM 替身：只实现 `ui.js` 的 `el()`/`discBadge` 用到的接口。
 * ⚠️ 节点**必须** `instanceof Node` —— `append`（`ui.js`）用该判别决定「元素」还是「文本」，
...
describe("D5 · discBadge / discChip 的空值守卫（无字面量 \"null\"）", () => {
	...
	it("⚠️ 反向哨兵：把 null 交进 append 风格的容器不会产生字面量 \"null\"", () => {
		// 这里复刻公共层 helper 的真实守卫条款：`el` 与 `append`（`ui.js`）都显式 skip null。
