...
describe("ExtensionRunner", () => {
	...
	const extensionActions: ExtensionActions = {
		sendMessage: () => {},
		sendUserMessage: () => {},
		startLiveMessage: () => ({ update: () => {}, end: async () => {} }),
		appendEntry: () => {},
		setSessionName: () => {},
	...
	describe("settings", () => {
		it("exposes the merged settings snapshot on ctx.settings", async () => {
			const result = await discoverAndLoadExtensions([], tempDir, tempDir);
			const sm = SettingsManager.inMemory({ userName: "甲" }, { overlay: { userName: "丙" } });
			const runner = new ExtensionRunner(
				result.extensions,
				result.runtime,
				tempDir,
			...
			expect(runner.createContext().settings.userName).toBe("丙");
			// 返回对象是 clone：篡改返回值不泄漏回 manager
			const ctx = runner.createContext();
			(ctx.settings as { userName?: string }).userName = "篡改";
			expect(runner.createContext().settings.userName).toBe("丙");
			// getter 调用时解析：合并源更新后新访问拿到新值
			sm.applyOverrides({ userName: "丁" });
			expect(runner.createContext().settings.userName).toBe("丁");
		});
	});
