...
describe("regression #2835: tool allowlists filter extension tools", () => {
	...
	it("allows only explicitly listed built-in and extension tools", async () => {
		...
		expect(compiledPrompt).toContain("- read: Read file contents");
		expect(compiledPrompt).toContain("- dynamic_tool: Run dynamic test behavior");
		expect(compiledPrompt).not.toContain("- bash:");
		expect(compiledPrompt).not.toContain("- edit:");
		session.dispose();
	});
	...
	it("disables all tools when the allowlist is empty", async () => {
		...
		expect(compiledPrompt).not.toContain("dynamic_tool");
		session.dispose();
	});
});
