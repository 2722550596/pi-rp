...
- **Tool Isolation**: Subagents run with a minimal default tool set (`read`, `grep`, `find`, `ls`, `bash`) intersected with the preset's tool policy. The default includes `bash`, so the tool set is read-only only when the preset's `tools` policy denies write tools. In peer-completion mode, the parent session's extension tools and UI context are inherited: subagents carry the parent's tool definitions and `uiContext` (so `ctx.ui.notify` and dialogs reach the parent UI), and tool-level event handlers (`tool_call`, `tool_result`, `tool_execution_*`) are forwarded from the parent's extensions. Session-level lifecycle events (`agent_start`, `session_start`, `before_agent_start`) remain isolated to avoid polluting the subagent's sealed prompt and context. Preset tool policies (`allow`/`deny`) apply to inherited extension tools just like built-in ones, so a deny-all profile still ends up with no tools.
...
- **Peer completion (with parent session)** — when invoked from a parent session (always the case via the `subagent` tool and `/subagent` command), the subagent inherits from the parent:
  ...
  - **Extension tools & UI context** — the parent session's extension tool definitions, UI context (`ctx.ui`), and tool event handlers (`tool_call`, `tool_result`, `tool_execution_*`) are inherited (see [Delegation Features](#delegation-features)); tool names join the default tool set and their snippets render in the `tools` slot unless the preset's tool policy filters them out.
