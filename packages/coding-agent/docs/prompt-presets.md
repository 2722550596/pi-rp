...
Compaction summaries are generated by sending prompts to the LLM. All four can be overridden:
...
The three compaction prompt fields support two placeholders that are replaced at runtime:

| Placeholder | Replaced with | Available in |
|---|---|---|
| `{conversation}` | The serialized conversation (formatted as `[User]:`, `[Assistant]:`, `[Tool result]:` lines) | `initialPrompt`, `updatePrompt`, `turnPrefixPrompt` |
...
Any field left undefined falls through to the built-in default, which wraps the conversation in `<conversation>` tags and the previous summary in `<previous-summary>` tags.
## Examples
