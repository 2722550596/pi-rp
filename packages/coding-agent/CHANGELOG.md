...
- **Unified message content transformer API**: `pi.registerMessageContentTransformer()` extends the display-time projection pipeline from assistant/user messages to *all* framework-rendered text content, including the default rendering of custom messages (context carries `customType`). The existing `registerMarkdownTransformer()` is retained unchanged — legacy transformers are adapted so they keep their original contract (they never receive `custom` contexts) — and both feed one ordered pipeline (`getMessageContentTransformers()`). The pipeline is now actually wired into the TUI: assistant (incl. streaming `isStreaming` propagation), user, and default custom rendering all receive the collectors. Streaming custom messages receive the same `isStreaming` state transitions as assistant messages.
- **Streaming-tolerant XML-like tag projection**: new public utilities `scanXmlTags()` and `createXmlTagTransformer()` project XML-like tags in message content onto display text during streaming. The scanner is stateless (each frame re-scans the full message) and tolerant of incomplete input: unterminated tags become `pending` segments whose content is everything to EOF (converging to closed next frame), half-open headers stay plain text, only allow-listed tags are recognized, and attribute parsing tolerates double/single-quoted and bare values. Tag-specific renderers receive `{ attrs, content, closed, pending }` plus the transform context, enabling e.g. `<speak from="elena" emo="cold">…</speak>` to stream as styled character dialogue with a pending cursor.
- **Async prompt preset slots**: `SlotRenderer` may now return a `Promise<string>` and a slot definition can declare `async: true`, letting custom slots do real I/O (subprocess, fetch, database queries) without blocking the event loop. Presets containing async slots compile through the async `compileMessages` path, which renders slot items **in parallel** when no side-effecting macros (`{{setvar}}`/`{{addvar}}`) are present; the synchronous fast path (`compileMessagesSync`) renders async slots as empty with an info diagnostic. `compileMessages`, `compileSystemPrompt`, `getPresetInjectMessages`, `prepareSubagentConversation`, and the extension `compilePreset` API are now async.
...
- Presets without a `chat-history` slot no longer silently drop the conversation: the real messages are appended at the end of the compiled array as implicit history (source kind `implicit-history`). The exception is the stateless one-shot pattern that re-inserts the latest user message via `{{lastUserMessage}}` without a slot, where history stays omitted to avoid duplication.
...
- Added the `append_message` RPC command: appends a custom message entry to the session's current active leaf and returns the persisted entry id. It is append-only (no caller-specified parent or entry id, no record modification) and carries no customType policy — the type policy is registered by the process's extensions or falls back to the default custom message semantics (user role in LLM context, `display: false` hides from the TUI), so a `--no-extensions` character subprocess can still read its own profile/situation memory.
...
- Added `compaction.summaryMaxTokens` (ordinary and per-model via `compaction.modelOverrides`) to set an explicit summarization output cap. Without it the cap stays derived as `0.8 * reserveTokens`, which couples the compaction threshold to the summary's output room and truncates reasoning-heavy summaries on models where thinking shares the output budget.
...
- **Edge-level disclosure in the memory system**: the "when to recall" condition is a first-class field again. A node's condition can now be overridden *per alias*, so one node reached through two names can carry two different trigger conditions. `MemoryStore.effectiveDisclosure(uri)` is the single read entry point and mirrors `resolveUri` (a `nodes.uri` hit uses `nodes.disclosure`; an alias hit uses `aliases.disclosure ?? nodes.disclosure`), so a read and a write can never disagree about which layer owns a uri. `nodes.disclosure` remains the canonical-entry condition; `edges.disclosure` is a pi-rp extension for association edges and is deliberately *not* part of the fallback chain (`aliases.disclosure` is the true counterpart of nocturne's `edge.disclosure`, since nocturne has no separate aliases table).
- New `MemoryStore` surface: `effectiveDisclosure`, `resolveEntry`, `hasAliasRow`, `setEntryDisclosure`, and `listAliasEntries` (which now carries `dead`). `revise` accepts a `when` field and writes the **structural** layer (an alias row when the uri has one, else the node); an empty `when` normalises to NULL at every write entry point (`memorize`/`revise`/`associate`/`consolidate`) so it can never sever the `??` fallback. `associate`'s documented `when` parameter is now actually wired through instead of being silently dropped.
- Disclosure is indexed for retrieval: `node_fts` gained a third column and is scored with `bm25(node_fts, 0.0, 1.0, 1.0)` (the `1.0` is FTS5's default, so existing ranking is unchanged). Entry-level conditions stay out of the FTS index by design.
...
- HTML export now replays tool calls with `argsComplete: false`. A live preview reads the current file or store, so in an export it would show post-hoc state rather than what the session saw; the authoritative diff still arrives via `details`. This affects every tool with a live preview, not only the memory ones.
...
- Fixed `relocate` dropping an alias's disclosure: a moved alias now inherits the **pre-move** `effectiveDisclosure` of its old uri (or the explicitly passed `when`), and the relocation statements list the disclosure column explicitly.
- Fixed `addAlias` not writing an audit event, unlike `addEdge`.
- Fixed `_updateNode` skipping the FTS re-index when only the disclosure changed, so a condition-only `revise` left the search index stale.
...
- Fixed context-only custom messages sent with `triggerTurn: false` while the agent was running being appended between an assistant tool call and its result, which providers that validate message order reject on replay. They are now queued and flushed once the turn's tool results are in ([#8537](https://github.com/earendil-works/pi/issues/8537)).
- Fixed automatic threshold compaction being skipped when the provider reported no streaming usage, which let context exceed the window until a hard overflow. The message-size estimate now drives the check when no usage-backed estimate is available ([#8328](https://github.com/earendil-works/pi/issues/8328)).
- Fixed truncated compaction and branch summaries (a `length` stop) being persisted as session checkpoints, which could replace real history with an incomplete summary ([#7048](https://github.com/earendil-works/pi/issues/7048)).
- Fixed branch summarization failing when reasoning tokens consumed a low model output cap; the summary request now raises its output budget up to 4096 tokens, capped by the model's maximum ([#8845](https://github.com/earendil-works/pi/issues/8845)).
...
- **Prompt preset system** — built-in prompt stack compiler, slot/macro engine, and JSON preset files under `.pi/prompt-presets/`. Presets replace Pi's built-in system prompt with a composable, reorderable definition and control message layout. Includes `/preset list`, `/preset use <id|none>`, `/preset reload`, and `/prompt` commands.
