...
**rerank 断路器(2026-09-22,benchmark 驱动)**:MIN_SCORE 阈值路线**判死**——fused 分数带完全重叠(联想 top1 0.65/0.76/0.85,跨域 0.44/0.60/0.79,后者 max 比前者 min 还高),bge 的语域相似度让任何解释型提问都能在思考型日记上拿 0.5+,纯阈值无法把相关与无关分开。落地 `BAAI/bge-reranker-v2-m3` 作**断路器而非重排器**:注入前对 fused top-8 的 500 字正文打 cross-encoder 分,`max < tau`(默认 `RECALL_BREAKER_TAU = 0.01`)→ 本 prompt 无记忆注入,审计记 `recall_breaker`。不重排的理由:联想 target 的 rerank 分中位数只有 0.063(cross-encoder 同样是表面匹配范式,看不出语义弧线——T-Mem 论点的另一面),重排+门槛会把联想 @3 从 73% 砍到 57%。benchmark(elias 库):跨域 15/15 全断(rerank 分精确 0);同域含真实锚的探针 0% 误断——「咖啡好像喝多了」被 elias 自己写的想起条件(`core://life/habits/self_neglect`「灌太多咖啡」)正确接住;联想组误断率 10%(τ=0.005)/17%(τ=0.01)。reranker 不可用时 **fail-open**(不改变任何现有行为)。配置 `memory.recall.breaker: { tau? }` 声明启用(每 prompt 多一次 provider 调用);模型可用 `memory.embeddings.rerankModel` 覆盖。复跑研究:`node --experimental-strip-types scripts/benchmark-recall.ts --db <库> --rerank`。

**注入选择器(2026-09-22,benchmark 驱动)**:断路器只回答「整页无关吗」,但真正每条消息都在发生的判断是「**这几条里哪些此刻该想起**」——RRF 的 TOP_K=3 是纯分数截断,会把同一故事线切碎、放进语域噪声。落地 TypeSafe Jev 作**逐条注入选择器**:`memory.recall.select: { tau? }`(opt-in,需 `TYPEAFE_API_KEY`)启用后 search 候选池放宽到 top-8,Jev 对每条 500 字正文判「此刻把这段记忆注入她的上下文,对这个对话有真实意义吗」,`noul ≥ tau`(默认 `RECALL_SELECT_TAU = 0.6`)才注入,仍受 TOP_K 封顶。elias 基准(10 query × top-4 = 40 判断):target 保持 **8/8 零错杀**;温水故事线 4 节点整线召回(人类想起温水事件就是整条线一起浮现,RRF 会切碎它);`freesia_trigger` 的 history/history_raw 重复域一放一压(0.85/0.20,自己发现内容重复);无关独白正确压制(「我是谁」在搬家话头下 0.35)。刻意的局限:Jev 的判别 margin 不足以做重排器(同线姊妹节点 0.87 与 target 0.86 难分)——它筛选,不排序。Jev 不可用/无 key 时 fail-open;筛选记 `recall_select` 审计。成本实测 ~0.9k tokens/判断,每 prompt 判 min(候选, 8) 条。复跑:`--dump` 导出候选后外部 judge 评分。
